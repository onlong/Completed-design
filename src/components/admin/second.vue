...
<el-table
    ...
    <el-drawer
      ...
      :visible.sync="drawer"
      :with-header="false">
      <div class="chouti" v-for="(item,index) in audit" :key="index">
        <div>日期：{{ item.date }}</div>
        <div class="jiany">建议：{{ item.opinion }}</div>
        <hr>
      </div>
...
<script>
import {getbookstate,getclass,getuserall,seeaudit,insertbook} from '../../api/axios'
  export default {
    methods: {
      see(row){
        seeaudit({bookid:row.id},this.tok).then(res=>{
          console.log('res');
          console.log(res);
          if(res.status){
            this.$message('数据获取失败')
    ...
    mounted(){
      ...
      getbookstate({stateid:1},this.tok).then(res=>{
        console.log('res');
        console.log(res);
        if(res.status){
            ...
            if(this.tableData[i].sign==1){
              this.tableData[i].sign='是'
            }else{
              this.tableData[i].sign="否"
            }
