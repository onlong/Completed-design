<template>
    <div class="box">
      ...
      <el-descriptions class="margin-top" title="" :column="3" :size="size">
        ...
        <el-descriptions-item label="是否约稿">{{row.invite}}</el-descriptions-item>
      ...
      <div>
        ...
        <div class="btn">
            <el-button @click="toopinion" type="success">发送意见</el-button>
...
<script>
import {getinviteid,getclass,releaseaudit,insertbook} from '../../../api/axios'
...
export default {
  beforeDestroy(){
    ...
    methods: {
    toopinion(){
        releaseaudit({nameid:this.row.writer,bookid:this.row.id,opinion:this.textarea},this.tok).then(res=>{
            if(res.status){
                this.$emit('change',3)
                insertbook({id:this.row.id,sign:0}).then(res=>{
      ...
      success(){
        insertbook({stateid:2,id:this.row.id}).then(res=>{
          if(res.status){
            this.$message('执行失败')
          }else{
            this.$emit('change',3)
