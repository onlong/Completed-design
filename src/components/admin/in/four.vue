<template>
    <div class="box">
      <el-form :model="row" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="篇幅" prop="long">
          <el-select v-model="row.long" placeholder="请选择需要篇幅">
        ...
        <el-form-item label="需要篇数" prop="num">
          <el-input-number v-model="row.num" @change="handleChange" :min="1"  label="描述文字"></el-input-number>
        </el-form-item>
        <el-form-item label="题材" prop="clessid">
          <el-select v-model="row.clessid" placeholder="请选择题材">
  ...
  <script>
    import {getclass,setinvite,ininvite} from '../../../api/axios'
    export default {
        props:['row'],
      data() {
        return {
      ...
      methods: {
        ...
        submitForm(formName) {
          this.$refs[formName].validate((valid) => {
            if (valid) {
                delete this.row.action
                ininvite(this.row,this.tok).then(res=>{
                if(res.status){
                  this.$message('修改失败')
                }else{
                  this.$message('修改成功')
                  this.$emit('change','5-1')
                }
              }).catch(err=>{
                console.log('err');
                console.log(err);
